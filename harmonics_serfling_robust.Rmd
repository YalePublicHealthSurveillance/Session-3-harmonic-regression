...
```{r loaddata}
# Load a dataset
us.pi <- read.csv("us_pi_mortality.csv")
...
# Check
serfling.ds[1:24, c('all','pred','winter','epi95','ex.all')]
...
excessN_serfling <- serfling.ds[excessN$season!='',] %>%
  group_by(season) %>%
  summarize('Excess_deaths_serfling'=round(sum(ex.all),-2),'N_epidemic_months'=sum(epi95)) 
